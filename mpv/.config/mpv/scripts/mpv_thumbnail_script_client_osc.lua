...
local user_opts = {
    ...
    barmargin = 50,              -- vertical margin of top/bottombar
    boxalpha = 70,              -- alpha of the background box,
                                -- 0 (opaque) to 255 (fully transparent)
    hidetimeout = 999999,       -- duration in ms until the OSC hides if no
...
local osc_styles = {
    ...
    topButtons = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs10\\fn" .. mp.get_property("options/osd-font") .. "}",

    elementDown = "{\\1c&H999999}",
    timecodes = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs19\\fn" .. mp.get_property("options/osd-font") .. "}",
    vidtitle = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs12\\q2\\fn" .. mp.get_property("options/osd-font") .. "}",
    box = "{\\rDefault\\blur0\\bord1\\1c&H2e2523\\3c&HFFFFFF\\fn" .. mp.get_property("options/osd-font") .. "}",

    topButtonsBar = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs18\\fn" .. mp.get_property("options/osd-font") .. "}",
    smallButtonsBar = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs20\\fn" .. mp.get_property("options/osd-font") .. "}",
    timecodesBar = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs27\\fn" .. mp.get_property("options/osd-font") .. "}",
    progressBar = "{\\blur0\\bord0\\1c&H839f08\\3c&HFFFFFF\\fs27\\fn" .. mp.get_property("options/osd-font") .. "}",
    progressBarBg = "{\\blur0\\bord0\\1c&H0f0c0b\\3c&HFFFFFF\\fs27\\fn" .. mp.get_property("options/osd-font") .. "}",
    timePosBar = "{\\blur0\\bord".. user_opts.tooltipborder .."\\1c&HFFFFFF\\3c&H2e2523\\fs30\\fn" .. mp.get_property("options/osd-font") .. "}",
    vidtitleBar = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs18\\q2\\fn" .. mp.get_property("options/osd-font") .. "}",

    wcButtons = "{\\1c&HFFFFFF\\fs24\\fn" .. mp.get_property("options/osd-font") .. "}",
    wcTitle = "{\\1c&HFFFFFF\\fs24\\q2\\fn" .. mp.get_property("options/osd-font") .. "}",
    tooltip = "{\\1c&HFFFFFF\\fs35\\q2\\fn" .. mp.get_property("options/osd-font") .. "}",
    wcBar = "{\\1c&H000000\\fn" .. mp.get_property("options/osd-font") .. "}",
}
...
local state = {
    ...
    osd = mp.create_osd_overlay("ass-events"),
		long_video_time_l = false,
		long_video_time_r = false,
}
        ...
        if (element.type == "slider") then
            ...
            if not (element.slider.tooltipF == nil) then
                ...
                if mouse_hit(element) then
                    ...
                    -- else
                    ty = element.hitbox.y1 + elem_geo.h/2 - 27 
                    -- end
                    ...
                    -- mpv_thumbnail_script.lua --
                    display_thumbnail({x=get_virt_mouse_pos(), y=ty+22, a=an}, sliderpos, elem_ass)
                    -- // mpv_thumbnail_script.lua // --
...
function bar_layout(direction)
    ...
    local padY = 9
    local buttonW = 20
    local tcW = (state.tc_ms) and 140 or 80 
    local tsW = 50
    ...
    osc_geo.w = osc_param.playresx + 4 - side_margin
    if direction < 0 then
    ...
    if direction > 0 then
        -- deadzone below OSC
      sh_area_y0 = user_opts.barmargin
      sh_area_y1 = (osc_geo.y + (osc_geo.h / 2)) + get_align(1 - (2*user_opts.deadzonesize), osc_param.playresy - (osc_geo.y + (osc_geo.h / 2)), 0, 0)
    else
        -- deadzone above OSC
      sh_area_y0 = get_align(-1 + (2*user_opts.deadzonesize), osc_geo.y - (osc_geo.h / 2), 0, 0)
      sh_area_y1 = osc_param.playresy  - user_opts.barmargin + 100
		end
    ...
    local count, limlist = limited_list('playlist', pos)
		...
		-- Left timecode
    tcW = (state.tc_ms or state.long_video_time_l) and 120 or 70 
    ...
    -- Track selection buttons
    	...
    	lo.style = osc_styles.smallButtonsBar
			...
			geo = { x = geo.x , y = geo.y, an = geo.an, w = tsW, h = geo.h }
		end
    ...
    -- Right timecode
    tcW = (state.long_video_time_r) and 140 or 70 
    ...
    lo.geometry = geo
    lo.layer = 15
    lo.style = osc_styles.progressBarBg
    lo.alpha[1] =
    ...
    lo.geometry = geo
    lo.style = osc_styles.progressBar
    lo.slider.border = 0
    lo.slider.gap = 2
...
function osc_init()
    ...
    ne.content = function ()
				local val = 0
        if (state.tc_ms) then
            val = (mp.get_property_osd("playback-time/full"))
        else
            val = (mp.get_property_osd("playback-time"))
        end
				val = val:gsub("^00:","")

				local old = state.long_video_time_l 
				state.long_video_time_l = #val > 6 

				if old ~= state.long_video_time_l then
						request_init_resize()
				end
    ...
    ne.content = function ()
        ...
        if (state.rightTC_trem) then
            if state.tc_ms then
                val = (mp.get_property_osd("playtime-remaining/full"))
            else
                val = (mp.get_property_osd("playtime-remaining"))
            end
						val = ""..val:gsub("^00:","")
        else
            if state.tc_ms then
                val = (mp.get_property_osd("duration/full"))
            else
                val = (mp.get_property_osd("duration"))
            end
						val = val:gsub("^00:","")
        end

				local old = state.long_video_time_r 
				state.long_video_time_r = #val > 8 

				if old ~= state.long_video_time_r then
						request_init_resize()
				end
				return val
    end
